...
#include <bits/stdc++.h> 
long long int robLinear(vector<int>& valueInHouse){
    int n=valueInHouse.size();
    if(n==1){
        return valueInHouse[0];
    }
    long long int prev2=valueInHouse[0];
    long long int prev=max(valueInHouse[0],valueInHouse[1]);
    for(int i=2;i<n;i++){
        long long int curr=max(prev,prev2+valueInHouse[i]);
        prev2=prev;
        prev=curr;
    }
    return prev;
...
long long int houseRobber(vector<int>& valueInHouse)
{
    ...
    for(int i=0;i<n;i++){
        if(i!=0){
            notF.push_back(valueInHouse[i]);
        }
        if(i!=n-1){
            notL.push_back(valueInHouse[i]);
        }
    }
    return max(robLinear(notF),robLinear(notL));
}